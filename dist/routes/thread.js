...
app
    ...
    try {
        ...
        const result = await db
            ...
            .from(conversations)
            .where(and(eq(conversations.userId, user.id), eq(conversationMessages.role, 'user')))
            .fullJoin(conversationMessages, and(eq(conversations.id, conversationMessages.conversationId), eq(conversationMessages.role, 'user')))
            .orderBy(desc(conversations.id))
            .limit(10);
        return c.json(result, 200);
    }
