...
app.post("/message", async (c) => {
    try {
        ...
        const telegramBotApiToken = headers["x-telegram-bot-api-secret-token"];
        ...
        if (photoTelegram) {
            ...
            try {
                const fileInfo = await ky
                    .get(`https://api.telegram.org/bot${tokenTelegram}/getFile?file_id=${photoTelegram.at(-1)?.file_id}`)
                    .json();
                ...
                const responseAgent = await (await agent).invoke({ messages: message }, { configurable: { thread_id: chatId } });
        ...
        const responseAgent = (await agent).invoke({ messages: humanMessage }, { configurable: { thread_id: chatId } });
